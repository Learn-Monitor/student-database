...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function mountEnrollment(catalog, currentScope, mount=root) {
        ...
        const prepareTeachersButton=action(panel,'Lehrkräfte-Tabelle aufbauen',async()=>{
            ...
            const individualSubjects=data.subjects.filter(s=>(s.mode||"REGULAR")==="INDIVIDUAL"&&s.assignmentGroup); if(individualSubjects.length){const area=el("section");area.className="individual-grade-teacher-mapping";area.append(el("h4","Lehrkräfte für individuelle Fächer"),el("p","Diese Zuordnung gilt für den gesamten Jahrgang im gewählten Halbjahr."));const table=el("table"),head=table.insertRow();["Fach","Zuordnungsgruppe","Lehrkraft"].forEach(t=>head.append(el("th",t)));const mappings=[];for(const subject of individualSubjects){const tr=table.insertRow();tr.insertCell().textContent=subject.name;tr.insertCell().textContent=subject.assignmentGroup==="RELIGION_ETHIK"?"Religion / Ethik":"WPF";const cell=tr.insertCell();const input=teacherChoice(cell,selectedGrade,subject.id);const existing=data.teaching.find(t=>t.grade===selectedGrade&&t.subjectId===subject.id&&t.semesterId===semesterId);if(existing)input.value=String(existing.teacherId);mappings.push({subject,input});}area.append(table);mappingArea.append(area);action(area,"Individuelle Fach-Lehrkräfte speichern",async()=>{const selected=mappings.filter(m=>m.input.value);if(!selected.length)throw Error("Bitte mindestens eine Lehrkraft auswählen.");for(const m of selected)await call("/assign-individual-grade-teacher",{grade:selectedGrade,semesterId,subjectId:m.subject.id,teacherId:Number(m.input.value)},"curriculum_manage_enrollment");status.textContent="Lehrkräfte für individuelle Fächer gespeichert.";});}
        ...
        const gradeTeacher=(grade,subjectId,semesterId)=>data.teaching.find(t=>t.grade===grade&&t.subjectId===subjectId&&t.semesterId===semesterId)?.teacherId||null; const teacherLabel=teacherId=>{const teacher=data.teachers.find(t=>t.id===teacherId);return teacher?teacher.first_name+" "+teacher.last_name:null;}; const renderIndividualTeacher=(cell,grade,subjectId,semesterId)=>{cell.replaceChildren();const teacherId=gradeTeacher(grade,subjectId,semesterId);cell.append(el("span",teacherId?teacherLabel(teacherId):"Keine Lehrkraft zugeordnet"));return teacherId;};
        const loadRoster=async()=>{
            ...
            const table=document.createElement("table");table.className="curriculum-wpf-table";const head=table.insertRow();["Schüler","Individuelles Fach","Zuständige Lehrkraft"].forEach((t,i)=>{const th=document.createElement("th");th.textContent=t;if(i<2){th.className="sortable";}head.append(th)});const rows=[];
            for(const student of pupils){const tr=table.insertRow();tr.insertCell().textContent=student.first_name+" "+student.last_name;const wpf=choice(tr.insertCell(),selectedGroup,[{id:0,name:"Noch nicht zugeordnet"},...subjects]);wpf.value=String(student.subjectId||0);const teacherCell=tr.insertCell();const teacher=renderIndividualTeacher(teacherCell,data.classes.find(c=>c.id===classId)?.grade,Number(wpf.value),semesterId);rows.push({student,tr,wpf,teacherCell,teacher});wpf.addEventListener("change",()=>{rows.find(r=>r.student.id===student.id).teacher=renderIndividualTeacher(teacherCell,data.classes.find(c=>c.id===classId)?.grade,Number(wpf.value),semesterId);});}
            ...
            action(board,"Zuordnungen speichern",async()=>{for(const row of rows){const subjectId=Number(row.wpf.value);if(!subjectId)continue;if(!row.teacher)throw Error("Bitte zuerst unter Lehrkräfte für individuelle Fächer eine Lehrkraft zuordnen.");await call("/assign-curriculum-wpf",{studentId:row.student.id,subjectId,classId,semesterId,assignmentGroup:selectedGroup,expectedSubjectId:row.student.subjectId??null},"curriculum_manage_enrollment");}status.textContent="Individuelle Fach-Zuordnung gespeichert.";});
        };
