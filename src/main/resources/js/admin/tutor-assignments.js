(() => {
  ...
  if (!root) return;
  const content = root.querySelector('[data-tutor-content]') || root;
  const status = root.querySelector('[data-tutor-status]');
  ...
  async function init() {
    if (status) status.textContent='Tutorzuordnungen werden geladen …';
    ...
    if(!semesters.length || !classes.length) { if (status) status.textContent='Für Tutorenzuordnungen werden ein Halbjahr und normale Klassen benötigt.'; return; }
    ...
    semester.addEventListener('change',()=>render().catch(error=>showError(error))); content.append(el('label','Halbjahr '),semester,table); await render();
    if (status) status.textContent='';
  }
  function showError() {
    if (status) status.textContent='Tutorzuordnungen konnten nicht geladen werden.';
    else content.replaceChildren(el('p','Tutorzuordnungen konnten nicht geladen werden.'));
  }
